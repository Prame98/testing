...
export const userSignup = (userInfo, userType) => {
  ...
  if (!userInfo.address) {
    return Promise.resolve('주소 정보가 올바르지 않습니다.');    
  }
  ...
  return instance.get(`/api/auth/checkId?id=${userId}`)
    .then((response) => {
      if (response.status === 200) {
        ...
        return Promise.resolve('사용할 수 있는 아이디입니다.');
      }
      return response;
    })
    .catch((error) => {
      if (error.response) {
        ...
        if (error.response.status === 409) {
          return Promise.resolve('이미 가입한 아이디 입니다.');
        } else {
          return Promise.resolve('아이디 중복 확인 중 에러가 발생했습니다.');
        }
      } else {
        console.error('응답 없음:', error.message);
        return Promise.resolve('서버와 통신할 수 없습니다.');
      }
    });
};
