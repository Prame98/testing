...
import { formatContentForTextarea, getImageSrc } from '../utils/commonUtils';
...
function BoardUpdate() {
  ...
  const updateStateFromData = (data) => {
    ...
    setContent(formatContentForTextarea(data.content));
    setPreview(getImageSrc(data.image));
  ...
  // * 게시글 상세 조회
  const { data,  isLoading ,isError }= useQuery(["boardUpdate", currentBoardId], () => getBoardDetail(currentBoardId),
    {
      enabled: !!currentBoardId, // currentBoardId가 존재할 때만 쿼리 실행
      staleTime: Infinity,
      onSuccess: (data) =>updateStateFromData(data),
    }
  );
  ...
  console.log("썸네일 이미지 업로드 :");
    ...
    if (newFile.type !== "image/jpeg" && newFile.type !== "image/png") {
      showAlert({ message: "jpg, png 형식의 이미지 파일을 업로드해주세요." });
      return;
    } else {
  ...
  const onTitleChange = (e) => {
    if (e.target.value.length > 30) {
      showAlert({ message: "제목은 30자 이상 입력할 수 없습니다." });
      return;
    }
  ...
  const onSubmitClick = (e) => {
    ...
    if (
      ...
      showAlert({ message: "모든 내용을 입력해주세요." });
      return;
    }
    ...
    console.log("**수정할 데이터 :", boardFormData);
  ...
  const submitBoardMutaion = useMutation(submitBoard, {
    onSuccess: (response) => {
      ...
      showAlert({ message: "게시글 작성이 완료되었습니다" });
  ...
  const editBoardMutation = useMutation(setEditBoard, {
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(["boardUpdate", currentBoardId]);
      
      showAlert({ message: "게시글 수정이 완료되었습니다." });
