...
function Navbar() {
  ...
  let personPage='/MyPageCustomer';
  if(userTypeCheck()==='owner'){
    personPage='/MyPage';
  }
  ...
  return (
    <StNavbar>
      <Flx>
        ...
        <button type='button' onClick={() => toggleIcon(IoPersonOutline, personPage)}>{location.pathname === personPage ? <IoPerson /> : <IoPersonOutline />}</button>
      </Flx>      
    </StNavbar>
  )
}
